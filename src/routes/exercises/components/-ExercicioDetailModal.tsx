    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <h2 className="text-lg font-bold text-text truncate pr-8">{exercicio.name}</h2>
        ...
        <div className="flex flex-col gap-3">
          ...
          {exercicio.equipment && (
            <div>
              ...
              <p className="text-sm text-text">{exercicio.equipment}</p>
            </div>
          )}
          {exercicio.instructions && exercicio.instructions.length > 0 && (
            <div>
              <p className="text-[10px] font-bold text-text-subtle uppercase tracking-wider mb-1">INSTRUÇÕES</p>
              <ol className="list-decimal list-inside space-y-1.5">
                {exercicio.instructions.map((inst: string, i: number) => (
